...
import { subDays, subWeeks, subMonths, format, eachDayOfInterval, startOfWeek, endOfWeek, startOfMonth, endOfMonth, eachWeekOfInterval, eachMonthOfInterval } from 'date-fns';
...
router.get('/classes', (req, res) => {
  ...
  Attendance.distinct('student.class')
    ...
    .catch(error => {
      ...
      res.status(500).json({ error: 'Failed to fetch class list', details: error.message });
    });
});
  ...
  Attendance.find(query)
    .then(students => {
      const results = students.map(s => {
        ...
        const present = day && day.subjects ? day.subjects.some(subject => subject.present) : false;
        return {
          _id: s.student?._id || s._id,
          name: s.student?.name || 'Unknown',
          rollNo: s.student?.rollNo || 'N/A',
          class: s.student?.class || 'N/A',
          present,
          subjects: day && day.subjects ? day.subjects : []
        };
      });
      res.json(results);
    })
    .catch(e => {
      console.error('Error fetching students with attendance:', e);
...
router.get('/attendance-stats', (req, res) => {
  ...
  Attendance.find(query)
    .then(students => {
      ...
      students.forEach(s => {
        const day = s.attendance.find(a => a.date === date);
        if (day && day.subjects && day.subjects.some(subject => subject.present)) presentToday++;
      });
...
router.get('/class-attendance-summary', (req, res) => {
  ...
  Attendance.find({})
    .then(students => {
      // Get all classes
      const classes = [...new Set(students.map(s => s.student?.class).filter(Boolean))];
      ...
      const result = classes.map(cls => {
        ...
        classStudents.forEach(s => {
          const day = s.attendance.find(a => a.date === date);
          if (day && day.subjects && day.subjects.some(subject => subject.present)) present++;
        });
    ...
    .catch(e => {
      console.error('Error summarizing class attendance:', e);
      res.status(500).json({ error: 'Failed to summarize class attendance', details: e.message });
    });
});
...
router.get('/attendance-trends', async function (req, res) {
  ...
  try {
    if (viewType === 'daily') {
      ...
      // Fallback if eachWeekOfInterval doesn't exist
      if (typeof eachWeekOfInterval === 'function') {
        dates = eachWeekOfInterval({ start: subMonths(now, 1), end: now });
      } else {
        // Manual weekly generation
      ...
      // Fallback if eachMonthOfInterval doesn't exist
      if (typeof eachMonthOfInterval === 'function') {
    ...
    // Fallback to daily if date generation fails
    dates = eachDayOfInterval({ start: subDays(now, 6), end: now });
  }
