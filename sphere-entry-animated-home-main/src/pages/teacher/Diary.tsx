...
//     try {
//       if (editingId) {
//         await api.put(`/AddDiary/${editingId}`, formData);
//         setEntries((prev) =>
//           prev.map((entry) =>
...
//       } else {
//         const response = await api.post(`/AddDiary`, formData);
...
const Diary = () => {
  ...
  const handleEdit = (entry) => {
    setFormData({
      date: entry.date,
      class: entry.class,
      section: entry.section || '', // Added section field
      subject: entry.subject,
      notes: entry.notes,
    });
    setEditingId(entry._id);
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                    <div className="flex items-start sm:items-center space-x-4">
                      ...
                      <div>
                        <h3 className="font-semibold">{entry.date}</h3>
                        <p className="text-sm text-gray-600">
                          {entry.class}{entry.section ? `-${entry.section}` : ''} • {entry.subject}
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-2">
