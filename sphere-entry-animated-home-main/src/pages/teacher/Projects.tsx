...
const Projects = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title || !formData.className || !formData.subject || !formData.dueDate || !formData.description) {
    ...
    try {
      if (editingId) {
        await api.put(`/AddProject/${editingId}`, newData);
        toast.success('Project updated successfully!');
      } else {
        await api.post(`/AddProject`, newData);
        toast.success('Project assigned successfully!');
      }

      fetchProjects();
      setFormData({ title: '', className: '', subject: '', dueDate: '', description: '' });
  ...
  const handleEdit = (project) => {
    setFormData({
      title: project.title,
      className: project.className || project.class,
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/AddProject/${id}`);
                  ...
                  {projects.length > 0 ? (
                    projects.map((project) => (
                      <TableRow key={project._id}>
                        ...
                        <TableCell>{project.className || project.class}</TableCell>
