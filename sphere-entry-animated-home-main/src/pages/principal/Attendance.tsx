...
const AttendanceAnalytics = () => {
  ...
  useEffect(() => {
    async function fetchClasses() {
      try {
        ...
        toast.error(err?.response?.data?.error || 'Failed to fetch class list');
      } finally {
        setLoading(false);
      }
    }
    ...
    async function fetchAllData() {
      try {
        ...
        const [studentRes, statsRes, classAttRes, chartRes] = await Promise.all([
          api.get(`/attendance/students-with-attendance`, {
            params: { date, class: cls }
          }),
          api.get(`/attendance/attendance-stats`, {
            params: { date, class: cls }
          }),
          api.get(`/attendance/class-attendance-summary`, {
            params: { date }
          }),
          api.get(`/attendance/attendance-trends`, {
  ...
  const exportReport = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get(`/attendance/export-attendance`, {
        params: {
          type: exportType,
      ...
      toast.error(err?.response?.data?.error || 'Failed to export attendance data');
    }
    setLoading(false);
  };
